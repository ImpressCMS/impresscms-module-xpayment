 ...
 */	
	...
	case "invoices":	
		...
		case "resendnotice":
			...
			$icmsMailer->setTemplate('xpayment_invoice_reminder.tpl');
			$icmsMailer->setSubject(sprintf(_XPY_EMAIL_REMINDER_SUBJECT, $invoice->getVar('grand'), $invoice->getVar('currency'), $invoice->getVar('drawto')));
		...
		case "list":
			...
			xpayment_adminMenu(1, 'index.php?op=invoices&fct=list');
			...
			$criteria = $invoice_handler->getFiltericms_db_criteria_Item($filter);
			$ttl = $invoice_handler->getCount($criteria);
			
			
			$pagenav = new icms_view_PageNav($ttl, $limit, $start, 'start', 'limit='.$limit.'&sort='.$sort.'&order='.$order.'&filter='.$filter.'&op='.$_REQUEST['op'].'&fct='.$_REQUEST['fct']);
			...
			foreach (array(	'mode','invoicenumber','drawfor','drawto','drawto_email','amount','grand','shipping',
							...
							'collect','wait','offline','remittion','discount','discount_amount') as $id => $key) {
			...
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_invoice_list.html');
			break;
		case 'export':	
		...
		case 'view':
			...
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_invoice_view.html');
	...
	case "discounts":	
		...
		default:
		case "list":
			$discount_handler =& icms_getModuleHandler('discounts', 'xpayment');
			
			xpayment_adminMenu(1, 'index.php?op=invoices&fct=lists');
			...
			$criteria = $discount_handler->getFiltericms_db_criteria_Item($filter);
			...
			foreach($discounts as $iid => $discount) {
				$GLOBALS['xoopsTpl']->append('discounts', $discount->getValues());
			}
			//loadModuleAdminMenu(7);		
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_discounts_list.html');
			break;
		case "create":
			extract($_POST);
			if (intval($redeems)==0) {
	...
	case "tax":	
		...
		case "list":
			...
			xpayment_adminMenu(6, 'index.php?op=tax&fct=list');
			...
			foreach($rates as $id => $rate) {
				$GLOBALS['xoopsTpl']->append('rates', $rate->getValues());
			}		
			...
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_tax_list.html');
			break;
		case 'save':
			$autotax_handler =& icms_getModuleHandler('autotax', 'xpayment');
	...
	case "transactions":	
		...
		case "list":
			...
			xpayment_adminMenu(2, 'index.php?op=transactions&fct=list');
			...
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_transactions_list.html');
			break;
		case 'view':
			xpayment_adminMenu(2, 'index.php?op=transactions&fct=list');
			$invoice_transactions_handler =& icms_getModuleHandler('invoice_transactions', 'xpayment');
			...
			//loadModuleAdminMenu(2);
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_transactions_view.html');
			break;
		}
		break;	
	case 'gateways':
		...
		if (isset($_REQUEST['gid'])) {
			...
			if (is_object($gateway)) 
				include_once((ICMS_ROOT_PATH.'/modules/xpayment/class/gateway/'.$gateway->getVar('class').'/language/'.$GLOBALS['icmsConfig']['language'].'/'.$gateway->getVar('class').'.php'));
		}
		switch ($_REQUEST['fct'])
		{
		default:
		case "list":
			...
			xpayment_adminMenu(3, 'index.php?op=gateways&fct=list');
			...
			$gateways = icms_core_Filesystem::getDirList((ICMS_ROOT_PATH.'/modules/xpayment/class/gateway/'));
			...
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_gateways_list.html');
			break;
		...
		case "options":
			...
			$criteria = new icms_db_criteria_Item('gid',$_GET['gid']);
			
			$options = $gateways_options_handler->getObjects($criteria, true);
			foreach($options as $goid => $option) {
				$ret=$option->getValues();
				$ret['name'] = (defined($ret['name'])?constant($ret['name']):$ret['name']);
				$GLOBALS['xoopsTpl']->append('options', $ret);
			}
			//loadModuleAdminMenu(3);
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_gateways_options.html');
			break;
	...
	case "permissions":
		...
		xpayment_adminMenu(4, 'index.php?op=permissions&fct=list');
		
		$opform = new icms_form_Simple(_XPY_AM_PERM_FCT, 'actionform', 'index.php', "get");
		$op_select = new icms_form_elements_Select("", 'fct', $_REQUEST['fct']);
		...
		$opform->addElement(new icms_form_elements_Hidden('op', 'permissions'));
		$opform->display();
	...
	case "groups":	
		...
		case "officers":			
			...
			{
			case "brokers":
				$criteria = new icms_db_criteria_Item('mode', 'BROKERS');
				break;
			case "accounts":
				$criteria = new icms_db_criteria_Item('mode', 'ACCOUNTS');
				break;
			case "officers":
				$criteria = new icms_db_criteria_Item('mode', 'OFFICERS');
				break;
			}
			xpayment_adminMenu(5, 'index.php?op=groups&fct=brokers');
			...
			foreach($groups as $rid => $group) {
				$GLOBALS['xoopsTpl']->append('groups', $group->getValues());
			}
			...
			$opform = new icms_form_Simple(_XPY_AM_GROUP_FCT, 'actionform', 'index.php', "get");
			$op_select = new icms_form_elements_Select("", 'fct', $_REQUEST['fct']);
			$op_select->setExtra('onchange="document.forms.actionform.submit()"');
			$op_select->addOptionArray(array(
			...
			$opform->addElement($op_select);
			$opform->addElement(new icms_form_elements_Hidden('op', 'groups'));
			$GLOBALS['xoopsTpl']->assign('selectform', $opform->render());
			
			//loadModuleAdminMenu(5);
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_groups.html');
			break;
		case 'save':
			$groups_handler =& icms_getModuleHandler('groups', 'xpayment');
			if ($_REQUEST['rid']==0)
		...
		case 'edit':
			...
			$GLOBALS['xoopsTpl']->display('db:admin/xpayment_cpanel_groups_edit.html');
			break;
		case 'delete':
			if (!isset($_POST['confirm'])) {
				icms_confirm(array('confirm'=>true,'op'=>$_REQUEST['op'],'fct'=>$_REQUEST['fct'],'rid'=>$_REQUEST['rid']), $_SERVER['PHP_SELF'], _XPY_MSG_CONFIRM_DELETE);
				icms_cp_footer();
				exit(0);
			}
			$groups_handler =& icms_getModuleHandler('groups', 'xpayment');
	...
	default:
		...
		xpayment_adminMenu(0, 'index.php?op=dashboard');
		...
		break;	
	case "about":
		xpayment_adminMenu(8, 'index.php?op=about');
		$paypalitemno='XPAYMENTABOUT100';
		$aboutAdmin = new ModuleAdmin();
		$about = $aboutAdmin->renderabout($paypalitemno, false);
		$donationform = array(	0 => '<form name="donation" id="donation" action="http://www.chronolabs.coop/modules/xpayment/" method="post" onsubmit="return icmsFormValidate_donation();">',
								1 => '<table class="outer" cellspacing="1" width="100%"><tbody><tr><th colspan="2">'.constant('_XPY_AM_MAKE_DONATION').'</th></tr><tr align="left" valign="top"><td class="head"><div class="icms-form-element-caption-required"><span class="caption-text">Donation Amount</span><span class="caption-marker">*</span></div></td><td class="even"><select size="1" name="item[A][amount]" id="item[A][amount]" title="Donation Amount"><option value="5">5.00 AUD</option><option value="10">10.00 AUD</option><option value="20">20.00 AUD</option><option value="40">40.00 AUD</option><option value="60">60.00 AUD</option><option value="80">80.00 AUD</option><option value="90">90.00 AUD</option><option value="100">100.00 AUD</option><option value="200">200.00 AUD</option></select></td></tr><tr align="left" valign="top"><td class="head"></td><td class="even"><input class="formButton" name="submit" id="submit" value="'._SUBMIT.'" title="'._SUBMIT.'" type="submit"></td></tr></tbody></table>',
...
<!--//
function icmsFormValidate_donation() { var myform = window.document.donation; 
...
<!-- End Form Validation JavaScript //-->');
		...
		for($key=0;$key<=4;$key++) {
			switch ($key) {
				case 2:
					$donationform[$key] =  sprintf($donationform[$key], $GLOBALS['icmsConfig']['sitename'] . ' - ' . (strlen(icms::$user->getVar('name'))>0?icms::$user->getVar('name'). ' ['.icms::$user->getVar('uname').']':icms::$user->getVar('uname')), icms::$user->getVar('email'), ICMS_LICENSE_KEY, strtoupper($GLOBALS['xpaymentModule']->getVar('dirname')),  strtoupper($GLOBALS['xpaymentModule']->getVar('dirname')). ' '.$GLOBALS['xpaymentModule']->getVar('name'));
					break;
			}
		}
