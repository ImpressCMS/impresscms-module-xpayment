 ...
 * @version         $Id: IcmsCache.php 8066 2011-11-06 05:09:33Z beckmi $
 */
...
class IcmsCache
{
    ...
    {
        if (!class_exists('IcmsCache' . ucfirst($name))) {
    ...
    function engine($name = 'file', $settings = array())
    {
        ...
        $cacheClass = 'IcmsCache' . ucfirst($name);
        $_this =& IcmsCache::getInstance();
    ...
    {
        $key = substr(md5(ICMS_URL), 0, 8) . '_' . $key;
        $_this =& IcmsCache::getInstance();
        $config = null;
