 ...
 * @subpackage cake.cake.libs.cache
 */
class IcmsCacheFile extends IcmsCacheEngine
{
    /**
     ...
     *                prefix = string prefix for filename, default => icms_
    ...
    function init($settings = array())
    {
        ...
        $defaults = array('path' => ICMS_TRUST_PATH . '/caches/icms_cache' , 'extension' => '.php' , 'prefix' => 'icms_' , 'lock' => false , 'serialize' => false , 'duration' => 31556926);
        $this->settings = array_merge($defaults, $this->settings);
        if (!isset($this->file)) {
            include_once (dirname(dirname(__FILE__)).'/file/icmsfile.php');
            $this->file = IcmsFile::getHandler('file', $this->settings['path'] . '/index.html', true);
        }
    ...
    function read($key)
    {
        ...
        if (!empty($data) && !empty($this->settings['serialize'])) {
            ...
            if (is_array($data)) {
                include_once(dirname(dirname(__FILE__)).'/utility/icmsutility.php');
                $data = IcmsUtility::recursive('stripslashes', $data);
            }
