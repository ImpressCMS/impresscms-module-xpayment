...
class ModuleAdmin
{
    ...
    {
        global $icmsModule;
        $this->_obj =& $icmsModule;
        echo "<style type=\"text/css\" media=\"screen\">@import \"" . ICMS_URL . "/modules/".$this->_obj->getVar('dirname')."/class/moduleclasses/moduleadmin/css/admin.css\";</style>";
        $this -> loadLanguage();
    }
    ...
    function getVersion()
    {
        ...
        $version = ICMS_FRAMEWORKS_MODULEADMIN_VERSION;
        return $version;
    }
    ...
    function getReleaseDate()
    {
         ...
         */
        Include_once 'icms_version.php';
        $releasedate = ICMS_FRAMEWORKS_MODULEADMIN_RELEASEDATE;
        return $releasedate;
    }
    ...
    function loadLanguage() {
        ...
        if ( !file_exists($fileinc = ICMS_ROOT_PATH . "/modules/" . $this->_obj->getVar('dirname') . "/class/moduleclasses/moduleadmin/language/{$language}/main.php" )){
            if ( !file_exists($fileinc = ICMS_ROOT_PATH . "/modules/" . $this->_obj->getVar('dirname') . "/class/moduleclasses/moduleadmin/language/english/main.php" )){
                return false;
            }
        }
    ...
    {
        $path = ICMS_URL . "/modules/" . $this->_obj->getVar('dirname') . "/";
        $pathsystem = ICMS_URL . "/modules/system/";
        $adminmenu = $this->_obj->getAdminMenu();
        $ret = "<div class=\"rmmenuicon\">\n";
        foreach (array_keys( $adminmenu) as $i) {
            if ($adminmenu[$i]['link'] != 'admin/index.php'){
                $ret .= "<a href=\"../" . $adminmenu[$i]['link'] . "\" title=\"" . $adminmenu[$i]['title'] . "\">";
                $ret .= "<img src=\"" . $path . $adminmenu[$i]['icon']. "\" alt=\"" . $adminmenu[$i]['title'] . "\" />";
                $ret .= "<span>" . $adminmenu[$i]['title'] . "</span>";
                $ret .= "</a>";
            }
        }
        if ($this->_obj->getInfo('help')) {
            if (substr(ICMS_VERSION, 0, 9) >= 'ICMS 1.3'){
                $ret .= "<a href=\"" . $pathsystem . "help.php?mid=" . $this->_obj->getVar('mid', 's') . "&amp;" . $this->_obj->getInfo('help') . "\" title=\"" . _AM_SYSTEM_HELP . "\">";
                $ret .= "<img width=\"32px\" src=\"" . ICMS_URL . "/modules/" . $this->_obj->getVar('dirname') . "/class/moduleclasses/icons/32/help.png\" alt=\"" . _AM_SYSTEM_HELP . "\" /> ";
    ...
    {
        $path = ICMS_URL . "/modules/".$this->_obj->getVar('dirname')."/class/moduleclasses/icons/32/";
        switch ($position)
        {
            default:
            case "right":
    ...
    {
        $line = "";
        $path = ICMS_URL . "/modules/".$this->_obj->getVar('dirname')."/class/moduleclasses/icons/16/";
        switch ($type)
        {
            default:
            case "default":
    ...
    function renderIndex()
    {
        ...
        if ($this->_obj->getInfo('min_php') || $this->_obj->getInfo('min_icms') || !empty($this -> _itemConfigBoxLine)){
            $ret .= "<tr>\n";
            $ret .= "<td colspan=\"2\">\n";
            ...
            if ($dbarray[ICMS_DB_TYPE]) {
                ...
                $dbRequiredVersion = $dbarray[ICMS_DB_TYPE];
                ...
                if ($reqVer > $curVer) {
                    $ret .= "<span style='color : red; font-weight : bold;'><img src='" . $path . "off.png' >" . sprintf(ICMS_DB_TYPE.' '._AM_MODULEADMIN_CONFIG_DB, $dbRequiredVersion, $dbCurrentVersion) . "</span><br />\n";
                } else {
                    $ret .= "<span style='color : green;'><img src='" . $path . "on.png' >" . sprintf(strtoupper(ICMS_DB_TYPE).' '._AM_MODULEADMIN_CONFIG_DB, $dbRequiredVersion, $dbCurrentVersion) . "</span><br />\n";
                }
            }

            // icms version
            if ($this->_obj->getInfo('min_icms')){
                if (substr(ICMS_VERSION, 6, strlen(ICMS_VERSION)-6) < $this->_obj->getInfo('min_icms')){
                    $ret .= "<span style='color : red; font-weight : bold;'><img src='" . $path . "off.png' >" . sprintf(_AM_MODULEADMIN_CONFIG_ICMS, $this->_obj->getInfo('min_icms'), substr(ICMS_VERSION, 6, strlen(ICMS_VERSION)-6)) . "</span>\n";
                }else{
                    $ret .= "<span style='color : green;'><img src='" . $path . "on.png' >" . sprintf(_AM_MODULEADMIN_CONFIG_ICMS, $this->_obj->getInfo('min_icms'), substr(ICMS_VERSION, 6, strlen(ICMS_VERSION)-6)) . "</span>\n";
                }
                $ret .= "<br />";
            }
    ...
    function renderAbout($paypal = '', $logo_icms = true)
    {
        $path = ICMS_URL . "/modules/".$this->_obj->getVar('dirname')."/class/moduleclasses/icons/32/";
        ...
        $ret .= "<table>\n<tr>\n<td width=\"100px\">\n";
        $ret .= "<img src='" . ICMS_URL . "/modules/" . $this->_obj->getVar('dirname') . "/" . $this->_obj->getInfo('image') . "' alt='" . $this->_obj->getVar('name') . "' style='float: left; margin-right: 10px;' />\n";
        $ret .= "</td><td>\n";
        ...
        $language = $GLOBALS['icmsConfig']['language'];
        if ( !is_file( ICMS_ROOT_PATH . "/modules/" . $this->_obj->getVar("dirname") . "/language/" . $language . "/changelog.txt" ) ){
            $language = 'english';
        }
        $language = empty($language) ? $GLOBALS['icmsConfig']['language'] : $language;
        $file = ICMS_ROOT_PATH. "/modules/" . $this->_obj->getVar("dirname") . "/language/" . $language . "/changelog.txt";
        if ( is_readable( $file ) ){
            $ret .= utf8_encode(implode("<br />", file( $file ))) . "\n";
        }else{
            $file = ICMS_ROOT_PATH. "/modules/" . $this->_obj->getVar("dirname") . "/docs/changelog.txt";
            if ( is_readable( $file ) ){
                $ret .= utf8_encode(implode("<br />", file( $file ))) . "\n";
            }
        }         
        ...
        if ($logo_icms == true){
            ...
            $ret .= "<a href=\"http://www.impresscms.org\" target=\"_blank\"><img src=\"" . $path . "impresscms.png\" alt=\"ImpressCMS\" title=\"ImpressCMS\"></a>";
            $ret .= "</div>";
        }
        return $ret;
    }
