 ...
 * @package kernel
 */
class XpaymentDiscounts extends icms_core_Object
{
	var $_dHandler;
    ...
    function __construct()
    {
		...
		$this->_dHandler = icms_getModuleHandler('discounts', 'xpayment');
	}
	...
	public function getValues($keys = null, $format = 's', $maxDepth = 1) {
		...
		if ($this->getVar('uid')>0) {
			...
			if (strlen($user->getVar('name'))>0)
				$ret['user']['uid'] = '<a href="'.ICMS_URL.'/userinfo.php?uid='.$this->getVar('uid').'">'.$user->getVar('name'). ' ('.$user->getVar('uname').')</a>';
			else 
				$ret['user']['uid'] = '<a href="'.ICMS_URL.'/userinfo.php?uid='.$this->getVar('uid').'">'.$user->getVar('uname').'</a>';
		}
	...
	function sendReminderEmail() {
		...
		$icmsMailer->setTemplate('xpayment_discount_reminder.tpl');
		$icmsMailer->setSubject(str_replace('%discount', floor($this->getVar('discount')), str_replace('%left', floor($this->getVar('redeems')-$this->getVar('redeemed')), constant('_XPY_EMAIL_DISCOUNT_REMINDER_SUBJECT'))));
		
		$icmsMailer->assign('CODE', $this->getVar('code'));
		$icmsMailer->assign('DISCOUNT', $this->getVar('discount'));
		$icmsMailer->assign('REDEEMS', $this->getVar('redeems'));
		$icmsMailer->assign('REDEEMED', $this->getVar('redeemed'));
		$icmsMailer->assign('LEFT', $this->getVar('redeems')-$this->getVar('redeemed'));
		$icmsMailer->assign('VALID', ($this->getVar('validtill')==0?_XPY_AM_DISCOUNT_FOREVER:date(_DATESTRING,$this->getVar('validtill'))));
		$icmsMailer->assign('EMAIL', $this->getVar('email'));
		$icmsMailer->assign("SITEURL", ICMS_URL);
		$icmsMailer->assign("SITENAME", $GLOBALS['icmsConfig']['sitename']);
		
		$icmsMailer->setToEmails($this->getVar('email'));
		if (time()-$this->getVar('sent')>$GLOBALS['xpaymentModuleConfig']['reminder_resend_in']) {
			$icmsMailer->send();	
...
* @package kernel
*/
class XpaymentDiscountsHandler extends icms_ipf_Handler
{
	var $_mHandler;
	var $_uHandler;
	var $_mod;
    ...
    {
		$this->db = $db;
        parent::__construct($db, 'discounts', 'XpaymentDiscounts', "did", "code", 'xpayment');
        $this->_mHandler =& icms::handler('icms_member');
        $this->_uHandler =& icms::handler('icms_member_user');
    ...
    function sendDiscountCode($email, $validtill, $redeems, $discount, $prefix, $uid = 0, $sendemail=true) {
    	...
    	if ($uid>0) {
    		...
    		if (is_object(icms::$user)) {
    			$object->setVar('uid', icms::$user->getVar('uid'));
    		}
    	}
    ...
    public function insert(&$object, $force = true, $checkObject = true, $debug = false) {
    	...
    	if ($object->isNew()) {
    		...
    		if ($object->getVar('redeems')>0&&$object->getVar('discount')>0) {
	    		if ($did = parent::insert($object, $force, false, $debug)) {
	    							
					$icmsMailer =& new icms_messaging_Handler();
					//$icmsMailer->setHTML(true);
					$icmsMailer->setTemplateDir((ICMS_ROOT_PATH.'/modules/xpayment/language/'.$GLOBALS['icmsConfig']['language'].'/mail_templates/'));
					$icmsMailer->setTemplate('xpayment_discount_new.tpl');
					$icmsMailer->setSubject(str_replace('%discount', floor($object->getVar('discount')), str_replace('%left', ($object->getVar('validtill')==0?_XPY_AM_DISCOUNT_FOREVER:date(_DATESTRING,$object->getVar('validtill'))), constant('_XPY_EMAIL_DISCOUNT_SUBJECT'))));
					
					$icmsMailer->assign('CODE', $object->getVar('code'));
					$icmsMailer->assign('DISCOUNT', $object->getVar('discount'));
					$icmsMailer->assign('REDEEMS', $object->getVar('redeems'));
					$icmsMailer->assign('VALID', ($object->getVar('validtill')==0?_XPY_MF_DISCOUNT_FOREVER:date(_DATESTRING,$object->getVar('validtill'))));
					$icmsMailer->assign('EMAIL', $object->getVar('email'));
					$icmsMailer->assign("SITEURL", ICMS_URL);
					$icmsMailer->assign("SITENAME", $GLOBALS['icmsConfig']['sitename']);
					
					$icmsMailer->setToEmails($object->getVar('email'));
					
					if ($checkObject) {
						if (!$icmsMailer->send()) {
							parent::deleteAll(new icms_db_criteria_Item('did', $did), true);
							return false;
						} else 
							return $did;
					} else 
    		...
    		if ($object->vars['redeemed']['changed']==true&&$object->getVar('redeems')>$object->getVar('redeemed')) {
				...
				//$icmsMailer->setHTML(true);
				$icmsMailer->setTemplateDir((ICMS_ROOT_PATH.'/modules/xpayment/language/'.$GLOBALS['icmsConfig']['language'].'/mail_templates/'));
				$icmsMailer->setTemplate('xpayment_discount_more.tpl');
				$icmsMailer->setSubject(str_replace('%discount', floor($object->getVar('discount')), str_replace('%left', floor($this->getVar('redeems')-$this->getVar('redeemed')), constant('_XPY_EMAIL_DISCOUNT_MORE_REDEEMS_SUBJECT'))));
				
				$icmsMailer->assign('CODE', $object->getVar('code'));
				$icmsMailer->assign('DISCOUNT', $object->getVar('discount'));
				$icmsMailer->assign('REDEEMS', $object->getVar('redeems'));
				$icmsMailer->assign('REDEEMED', $object->getVar('redeemed'));
				$icmsMailer->assign('LEFT', $object->getVar('redeems')-$object->getVar('redeemed'));
				$icmsMailer->assign('VALID', ($object->getVar('validtill')==0?_XPY_MF_DISCOUNT_FOREVER:date(_DATESTRING,$object->getVar('validtill'))));
				$icmsMailer->assign('EMAIL', $object->getVar('email'));
				$icmsMailer->assign("SITEURL", ICMS_URL);
				$icmsMailer->assign("SITENAME", $GLOBALS['icmsConfig']['sitename']);
    	...
    	return parent::insert($object, $force, false, $debug);
    }
