...
class PaypalGatewaysPlugin {
	...
	function __construct($invoice, $gateway) {
    	...
    	if (file_exists((ICMS_ROOT_PATH.'/modules/xpayment/language/'.$GLOBALS['icmsConfig']['language'].'/paypal.php')))
    		include_once((ICMS_ROOT_PATH.'/modules/xpayment/language/'.$GLOBALS['icmsConfig']['language'].'/paypal.php'));
    	else 
    		include_once((ICMS_ROOT_PATH.'/modules/xpayment/language/english/paypal.php'));
	...
	function checkCustom($request) {
		if (!isset($request))
			$request=$_REQUEST;			
		return (sha1($this->_gateway->getVar('salt').$this->_invoice->getVar('key').$this->_invoice->getVar('iid').$this->_invoice->getVar('grand').ICMS_LICENSE_KEY)==$request['custom']);
	}
	...
	function calcCustom() {
		return sha1($this->_gateway->getVar('salt').$this->_invoice->getVar('key').$this->_invoice->getVar('iid').$this->_invoice->getVar('grand').ICMS_LICENSE_KEY);
	}
	
	// HTML GENERATION FUNCTIONS FOR PAYMENT FORM	
	function getPaymentHTML() {
		
		if ($GLOBALS['xpaymentModuleConfig']['feecomphensate']) {
			$invoice_transaction_handler = icms_getModuleHandler('invoice_transactions', 'xpayment');
		...
		if ($this->_gateway->getVar('testmode')==true)
			$html .= '<form action="'.$this->_gateway->_options['sandbox'].'" method="post">';
		else 
			$html .= '<form action="'.$this->_gateway->_options['url'].'" method="post">';

		$html .= '<input type="hidden" name="cmd" value="_xclick">';
		$html .= '<input type="hidden" name="no_note" value="1">';
		$html .= '<input type="hidden" name="return" value="'.ICMS_URL.'/modules/xpayment/return.php">';
		$html .= '<input type="hidden" name="cancel_url" value="'.ICMS_URL.'/modules/xpayment/cancel.php">';
		$html .= '<input type="hidden" name="notify_url" value="'.ICMS_URL.'/modules/xpayment/ipn.php">';
		...
		$html .= '<input type="hidden" name="invoice" value="'.$this->_invoice->getVar('iid').'-'.substr(md5(ICMS_LICENSE_KEY.date('Ymdhmi')),mt_rand(0,28),3).'">';
