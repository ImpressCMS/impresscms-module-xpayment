...
class TwocheckoutGatewaysPlugin {
	...
	function __construct($invoice, $gateway) {
    	...
    	if (file_exists((ICMS_ROOT_PATH.'/modules/xpayment/language/'.$GLOBALS['icmsConfig']['language'].'/twocheckout.php')))
    		include_once((ICMS_ROOT_PATH.'/modules/xpayment/language/'.$GLOBALS['icmsConfig']['language'].'/twocheckout.php'));
    	else 
    		include_once((ICMS_ROOT_PATH.'/modules/xpayment/language/english/twocheckout.php'));
	...
	function getPaymentHTML() {
		...
		$items = $invoice_items_handler->getObjects(new icms_db_criteria_Item('iid', $this->_invoice->getVar('iid')));
		...
		$html .= '<input type="hidden" name="x_Receipt_Link_URL" value="'.ICMS_URL.'/modules/xpayment/return.php">';
