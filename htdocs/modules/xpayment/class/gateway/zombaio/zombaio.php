...
class ZombaioGatewaysPlugin {
	...
	function __construct($invoice, $gateway) {
    	...
    	if (file_exists((ICMS_ROOT_PATH.'/modules/xpayment/language/'.$GLOBALS['icmsConfig']['language'].'/zombaio.php')))
    		include_once((ICMS_ROOT_PATH.'/modules/xpayment/language/'.$GLOBALS['icmsConfig']['language'].'/zombaio.php'));
    	else 
    		include_once((ICMS_ROOT_PATH.'/modules/xpayment/language/english/zombaio.php'));
	...
	function goIPN($request) {
		...
		$cfgSiteName	= $GLOBALS['icmsConfig']['sitename'];
	...
	function getPaymentHTML() {
		...
		$depositpercentile =0; 
		if ($GLOBALS['xpaymentModuleConfig']['depositcomphensate']) {
			$invoice_transaction_handler = icms_getModuleHandler('invoice_transactions', 'xpayment');
		...
		if ($this->_gateway->getVar('testmode')==true)
			$html .= '<form action="'.$this->_gateway->_options['url'].'?'.$this->_gateway->_options['siteid'].'.'.$this->_gateway->_options['pricingid'].'.'.$this->_gateway->_options['lang'].'" method="post">';
		else 
			$html .= '<form action="'.$this->_gateway->_options['url'].'?'.$this->_gateway->_options['siteid'].'.'.$this->_gateway->_options['pricingid'].'.'.$this->_gateway->_options['lang'].'" method="post">';

		$html .= '<input type="hidden" name="identifier" value="'.$this->_invoice->getVar('iid').'">';
		$html .= '<input type="hidden" name="approve_url" value="'.ICMS_URL.'/modules/xpayment/return.php">';
		$html .= '<input type="hidden" name="decline_url" value="'.ICMS_URL.'/modules/xpayment/cancel.php">';
