 ...
 * @package kernel
 */
class XpaymentInvoice_transactions extends icms_core_Object
{
	...
	function runPlugin() {
		...
		include_once((ICMS_ROOT_PATH.'/modules/xpayment/plugin/'.$invoice->getVar('plugin').'.php'));
...
* @package kernel
*/
class XpaymentInvoice_transactionsHandler extends icms_ipf_Handler
{
    function __construct(&$db) 
    {
		$this->db = $db;
        parent::__construct($db, 'invoice_transactions', "tiid", "transactionid", '', 'xpayment');
    }
    ...
    function countTransactionId($transactionid) {
    	$criteria = new icms_db_criteria_Item('transactionid', $transactionid);
    ...
    public function insert(&$obj, $force = true, $checkObject = true, $debug = false) {
		...
		@$invoice_handler->insert($invoice, true, $checkObject, $debug);
		if ($run_plugin==true)
			$obj->runPlugin();
		return $tiid;
    }
