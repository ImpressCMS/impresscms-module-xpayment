 ...
 */	
	...
	if (!in_array($GLOBALS['xpaymentModuleConfig']['officers'], icms::$user->getGroups())) {
		redirect_header(ICMS_URL.'/modules/xpayment/index.php', 3, _NOPERM);
		exit(0);
	}
	...
	case "invoices":	
		...
		case "list":
			...
			$criteria = new icms_db_criteria_Compo(new icms_db_criteria_Item('officer_uids', '%"'.icms::$user->getVar('uid').'"%', 'LIKE'));
			$criteria->add(new icms_db_criteria_Item('remittion', "('NOTICE', 'COLLECT', 'SETTLED', 'PENDING')", 'IN'));
